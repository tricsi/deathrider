...
import { audio, mixer, music, sound, SoundProps, wave } from "../../modules/audio";
import { emit, off, on } from "../../modules/event";
...
export class Loader extends Node2D
{
    ...
    constructor() {
        ...
        this.layer
            .add(this.txt)
            .add(new Overlay)
        on("click keydown", this.load, document)
        this.init()
    }
    ...
    load = async () => {
        off("click keydown", this.load, document)
        this.text = "LOADING..."
        ...
        await sound("death", ["sine", 0.3, [2, 0.4, 0]], [110, 15, 0])
